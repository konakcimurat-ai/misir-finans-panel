...
# -------------------------
# 🔒 PASSWORD PROTECTION (Secrets ile)
...
def check_password():
    ...
    if not app_pw:
        st.error('APP_PASSWORD tanımlı değil. Manage app → Settings → Secrets içine şunu ekle: APP_PASSWORD = "sifren"')
        st.stop()
...
st.sidebar.divider()
st.sidebar.header("🏭 Operasyonel Ayarlar (TÜM TABLARI ETKİLER)")
...
st.sidebar.caption("Aşağıdaki ayar BASE rakamlarını değiştirmez, sadece senaryo hesabına çarpan ekler.")
...
for ay in range(1, 13):
    ...
    kasa_base += net_b
    pnl_base.append({"Ay": f"Ay {ay}", "Gelir ($)": gelir_b, "Hammadde ($)": ham_b, "OPEX ($)": op_b,
                     "Yatırım ($)": aylik_yatirim_odeme, "Aylık Net ($)": net_b, "Kasa ($)": kasa_base})

    gelir_s = gelir_b * (1 + kar_marji)
    fire_katsayi = 1 / max((1 - fire_orani), 0.01)
    ham_s = ham_b * fire_katsayi
    op_s = op_b * (1 + opex_fire_esneklik * fire_orani)

    net_s = gelir_s - ham_s - op_s - aylik_yatirim_odeme
    kasa_scn += net_s
    pnl_scn.append({"Ay": f"Ay {ay}", "Gelir ($)": gelir_s, "Hammadde ($)": ham_s, "OPEX ($)": op_s,
                    "Yatırım ($)": aylik_yatirim_odeme, "Aylık Net ($)": net_s, "Kasa ($)": kasa_scn})
...
with t1:
    ...
    with col1:
        st.metric("TOPLAM CAPEX (BASE)", f"{toplam_capex:,.0f} $")
        base_ebt = df_pnl_base["Aylık Net ($)"].sum()
        scn_ebt = df_pnl_scn["Aylık Net ($)"].sum()
        base_roi = (base_ebt / toplam_capex) * 100 if toplam_capex > 0 else 0
        scn_roi = (scn_ebt / toplam_capex) * 100 if toplam_capex > 0 else 0
        st.metric("ROI (BASE)", f"{base_roi:.1f}%")
        st.metric("ROI (SENARYO)", f"{scn_roi:.1f}%", delta=f"{(scn_roi-base_roi):+.1f} puan")
        st.plotly_chart(px.pie(raw_df_c, values='Toplam (USD)', names='Kategori', title="Yatırım Kırılımı (BASE, kur dahil)"), use_container_width=True)
    with col2:
        st.dataframe(styled_df_c.style.apply(apply_styles, axis=1).format({"EGP": "{:,.0f}", "USD": "{:,.0f}", "Toplam (USD)": "{:,.2f} $"}), use_container_width=True, hide_index=True)

with t2:
    colA, colB = st.columns([1, 2])
    with colA:
        st.metric("Aylık Sabit Gider (BASE OPEX Tablosu)", f"{aylik_sabit_opex_usd_base:,.0f} $/ay")
        st.metric("Fire (SENARYO)", f"{fire_orani:.0%}")
        st.metric("Kâr Marjı (SENARYO)", f"{kar_marji:.0%}")
        st.plotly_chart(px.bar(raw_df_o, x='Kategori', y='Toplam (USD)', color='Kategori', title="Aylık Sabit Gider Dağılımı (BASE)"), use_container_width=True)
    with colB:
        st.dataframe(styled_df_o.style.apply(apply_styles, axis=1).format({"EGP": "{:,.0f}", "USD": "{:,.0f}", "Toplam (USD)": "{:,.2f} $"}), use_container_width=True, hide_index=True)
...
with t5:
    ...
    st.plotly_chart(
        go.Figure(go.Waterfall(
            ...
            y=[df_pnl_scn["Gelir ($)"].sum(), -df_pnl_scn["Hammadde ($)"].sum(), -df_pnl_scn["OPEX ($)"].sum(), -df_pnl_scn["Yatırım ($)"].sum(), 0]
